...
class GMLFieldsReportGeneratorOld:
                ...
                for field in field_layer.fields():
                    if field.comment() == 'EGB_JednostkaRejestrowaGruntow':
                        if len(self.project.mapLayersByName(field.comment())) == 0:
                            ...
                            if len(self.project.mapLayersByName('EGB_UdzialWlasnosci')) == 0:
                                self.showMessage('Zestawienie EGiB - działka',
                                            'Do projektu nie została wczytana warstwa EGB_UdzialWlasnosci', ownership_message)
                                ownership_message = False
                            else:
                                ownership_layer = self.project.mapLayersByName('EGB_UdzialWlasnosci')[0]
                                        ...
                                        if ownership_field.comment() == field.comment():
                                            for ownership_feature in ownership_layer.getFeatures(
                                                ...
                                                try:
                                                    ...
                                                    ownership_share = ''
                                                try:
                                                    ownership_single = ownership_feature.attribute("podmiotUdzialuWlasnosci_EGBPodmiot_osobaFizyczna5")
                                                except:
                                                    ownership_single = NULL
                                                try:
                                                    ownership_marriage = ownership_feature.attribute("podmiotUdzialuWlasnosci_EGBPodmiot_malzenstwo4")
                                                except:
                                                    ownership_marriage = NULL
                                                try:
                                                    ownership_institution = ownership_feature.attribute("podmiotUdzialuWlasnosci_EGBPodmiot_instytucja3")
                                                except:
                                                ...
                                                if ownership_single != NULL:
                                                    ...
                                                    if len(self.project.mapLayersByName("EGB_OsobaFizyczna")) == 0:
                                                        self.showMessage('Zestawienie EGiB - działka',
                                                                    'Do projektu nie została wczytana warstwa EGB_OsobaFizyczna', ownership_single_message)
                                                        ownership_single_message = False
                                                    else:
                                                        owner_layer = self.project.mapLayersByName("EGB_OsobaFizyczna")[0]
                                                    ...
                                                    if len(self.project.mapLayersByName("EGB_Malzenstwo")) == 0:
                                                        self.showMessage('Zestawienie EGiB - działka',
                                                                    'Do projektu nie została wczytana warstwa EGB_Malzenstwo', ownership_marriage_message)
                                                        ...
                                                        for owner_feature in owner_layer.getFeatures(
                                                                "gml_id IN(%s)" % owner_ids):
                                                            try:
                                                                marriage_first_partner = owner_feature.attribute("osobaFizyczna2")
                                                            except:
                                                                marriage_first_partner = ''
                                                            marriage_ids = "'%s'" % marriage_first_partner
                                                            try:
                                                                marriage_second_partner = owner_feature.attribute("osobaFizyczna3")
                                                            except:
                                                                marriage_second_partner = ''
                                                            marriage_ids = "%s, '%s'" % (
                                                                marriage_ids,
                                                                marriage_second_partner) if marriage_ids != NULL else "'%s'" % marriage_second_partner
                                                            if len(self.project.mapLayersByName("EGB_OsobaFizyczna")) == 0:
                                                                self.showMessage('Zestawienie EGiB - działka',
                                                                            'Do projektu nie została wczytana warstwa EGB_OsobaFizyczna', ownership_single_message)
                                                                ownership_single_message = False
                                                                ...
                                                                owner_sub_layer = \
                                                                self.project.mapLayersByName("EGB_OsobaFizyczna")[0]
                                                                owners = ''
                                                                for owner_sub_feature in owner_sub_layer.getFeatures(
                                                                    ...
                                                                    try:
                                                                        surname_first = owner_sub_feature.attribute("pierwszyCzlonNazwiska")
                                                                    except:
                                                                        surname_first = ''
                                                                    owner = '%s %s' % (
                                                                        owner, surname_first) if surname_first != NULL else owner
                                                                    try:
                                                                        surname_second = owner_sub_feature.attribute("drugiCzlonNazwiska")
                                                                    except:
                                                                        surname_second = ''
                                                                    owner = '%s %s' % (
                                                    ...
                                                    if len(self.project.mapLayersByName("EGB_Instytucja")) == 0:
                                                        self.showMessage('Zestawienie EGiB - działka',
                                                                    'Do projektu nie została wczytana warstwa EGB_Instytucja', ownership_institution_message)
                    ...
                    if field.comment() == 'EGB_Klasouzytek':
                        if len(self.project.mapLayersByName(field.comment())) == 0:
                            self.showMessage('Zestawienie EGiB - działka',
                                        'Do projektu nie została wczytana warstwa EGB_Klasouzytek', landuse_message)
                            landuse_message = False
                        else:
                            landuse_layer = self.project.mapLayersByName(field.comment())[0]
